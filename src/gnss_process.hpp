...
class GPSProcess
{
  public:
    ...
    void Initialize(const double lon_init, const double lat_init, const double alti_init);
    void Reset();
    ...
    Eigen::Vector3d ECEF2ENU(const Eigen::Vector3d &vel_ecef, double lat_ref, double lon_ref);
    void Process(const sensor_msgs::NavSatFix::ConstPtr &msg, GPSGroup &gps_out);
    void Process(const gnss_comm::GnssPVTSolnMsg::ConstPtr &msg, GPSGroup &gps_out);
    void uwb_process(const common_msgs::LinktrackNodeframe2::ConstPtr &msg, GPSGroup &gps_out);
...
void GPSProcess::set_gps_cov(const V3D &scaler)
{
    cov_gps = scaler;
}

Eigen::Vector3d GPSProcess::ECEF2ENU(const Eigen::Vector3d &vel_ecef, double lat_ref, double lon_ref)
{
    // 计算旋转矩阵的各个元素
    ...
    vel_enu = R * vel_ecef;
    // vel_enu.x = R[0][0] * vel_ecef.x + R[0][1] * vel_ecef.y + R[0][2] * vel_ecef.z;
    // vel_enu.y = R[1][0] * vel_ecef.x + R[1][1] * vel_ecef.y + R[1][2] * vel_ecef.z;
    // vel_enu.z = R[2][0] * vel_ecef.x + R[2][1] * vel_ecef.y + R[2][2] * vel_ecef.z;
...
{
    geo_converter.Forward(lla_[0], lla_[1], lla_[2], utm_enu[0], utm_enu[1], utm_enu[2]);
...
// TODO:
void GPSProcess::Initialize(const double lon_init, const double lat_init, const double alti_init)
{
    if (!proj_init)
    {
        lon_0 = lon_init;
...
void GPSProcess::Process(const sensor_msgs::NavSatFix::ConstPtr &msg, GPSGroup &gps_out)
{
...
void GPSProcess::Process(const gnss_comm::GnssPVTSolnMsg::ConstPtr &msg, GPSGroup &gps_out)
{
...
void GPSProcess::uwb_process(const common_msgs::LinktrackNodeframe2::ConstPtr &msg, GPSGroup &gps_out)
{
